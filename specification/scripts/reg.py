...
class Registry:
    ...
    def markTypeRequired(self, typename, required):
        ...
        if (type != None):
            if (required):
                ...
                # are emitted
                if ('bitvalues' in type.elem.attrib):
                    depType = type.elem.get('bitvalues')
    ...
    #
    # groupname - name of group (aka enum)
    # required - boolean (to tag features as required or not)
    def markGroupRequired(self, groupname, required):
        self.gen.logMsg('diag', '*** tagging enum:', groupname, '-> required =', required)
        group = self.lookupElementInfo(groupname, self.groupdict)
        if (group != None):
            group.required = required
        else:
            self.gen.logMsg('warn', '*** group:', groupname , 'IS NOT DEFINED')
    ...
    def generateFeature(self, fname, ftype, dictionary):
        ...
        #   the element, as well as many in embedded <type> and <enum> tags
        #   within the element.
        ...
        if (ftype == 'type'):
            ...
            if (f.elem.get('category') == 'enum'):
                ...
                if (group == None):
                    ...
                    genProc = None
                    self.gen.logMsg('warn', '*** NO MATCHING ENUM GROUP FOUND for {}!!!',fname)
                else:
